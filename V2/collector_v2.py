"""
CUA Collector V2 — Automated UI Action Data Collector
=====================================================
Fully automated version using the C++ capture engine (cua_capture).

Instead of manual Ctrl+F9 for each action, V2 continuously captures
screenshots into a ring buffer and automatically records all mouse/keyboard
...
Requirements:
  Build the C++ module first:
    cd V2/build && cmake .. && make -j$(nproc)
"""
...
try:
    ...
    if 'GLIBCXX' in err or 'libstdc++' in err:
        ...
        print("     LD_PRELOAD=/usr/lib/x86_64-linux-gnu/libstdc++.so.6 python collector_v2.py")
...
class StatusOverlay:
    ...
    def _run_tk(self):
        ...
        self._root = tk.Tk()
        self._root.title("CUA Collector V2")
...
class CollectorV2:
    ...
    def _handle_completed_action(self, action):
        ...
        print(f"   ✅ Action #{seq}: {action.type} @ ({action.x},{action.y}) [{status}]")
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description='CUA_Collector V2 - Automated UI Action Data Collector')
